...
const LATEST_ANDROID_APK_PART_URLS = [
  'https://raw.githubusercontent.com/SemGAA/Program_kinp/apk-downloads/downloads/parts/cinema-notes-1.5.9.part.000',
  'https://raw.githubusercontent.com/SemGAA/Program_kinp/apk-downloads/downloads/parts/cinema-notes-1.5.9.part.001',
  'https://raw.githubusercontent.com/SemGAA/Program_kinp/apk-downloads/downloads/parts/cinema-notes-1.5.9.part.002',
  'https://raw.githubusercontent.com/SemGAA/Program_kinp/apk-downloads/downloads/parts/cinema-notes-1.5.9.part.003',
  'https://raw.githubusercontent.com/SemGAA/Program_kinp/apk-downloads/downloads/parts/cinema-notes-1.5.9.part.004',
  'https://raw.githubusercontent.com/SemGAA/Program_kinp/apk-downloads/downloads/parts/cinema-notes-1.5.9.part.005',
  'https://raw.githubusercontent.com/SemGAA/Program_kinp/apk-downloads/downloads/parts/cinema-notes-1.5.9.part.006',
  'https://raw.githubusercontent.com/SemGAA/Program_kinp/apk-downloads/downloads/parts/cinema-notes-1.5.9.part.007',
];
const APP_SCHEME = 'cinemaapp';
const EMAIL_CODE_TTL_MS = 15 * 60 * 1000;
const EMAIL_CODE_RESEND_COOLDOWN_MS = 60 * 1000;
const MAX_AVATAR_URL_LENGTH = 1200000;
...
async function handleRequest(request, env) {
  ...
  if (
    pathname === '/downloads/cinema-notes-android.apk' &&
    (request.method === 'GET' || request.method === 'HEAD')
  ) {
    return handleLatestApkDownload(request);
  }
  ...
  if (watchShowMatch && request.method === 'DELETE') {
    return handleDeleteWatchRoom(env, auth.user, watchShowMatch[1]);
  }
...
async function handleLatestApkDownload(request) {
  const responseHeaders = {
    'Access-Control-Allow-Origin': '*',
    'Cache-Control': 'public, max-age=300',
    'Content-Disposition': 'attachment; filename=\"Cinema-Notes-1.5.9.apk\"',
...
async function handleDeleteWatchRoom(env, currentUser, code) {
  const room = await first(env.DB, 'SELECT * FROM watch_rooms WHERE code = ?', [String(code).trim().toUpperCase()]);
  if (!room) {
    return json({ message: 'Room not found.' }, 404);
  }
  ...
  if (Number(room.host_user_id) !== Number(currentUser.id)) {
    await run(env.DB, 'DELETE FROM watch_room_members WHERE watch_room_id = ? AND user_id = ?', [
      room.id,
      currentUser.id,
    ]);
    return json({ message: 'Комната убрана из вашего списка.' });
  }

  await run(env.DB, 'DELETE FROM watch_room_messages WHERE watch_room_id = ?', [room.id]);
  await run(env.DB, 'DELETE FROM watch_room_invites WHERE watch_room_id = ?', [room.id]);
  await run(env.DB, 'DELETE FROM watch_room_members WHERE watch_room_id = ?', [room.id]);
  await run(env.DB, 'DELETE FROM watch_rooms WHERE id = ?', [room.id]);

  return json({ message: 'Комната удалена.' });
}
